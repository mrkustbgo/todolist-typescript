...
const Main = () => {
  ...
  useEffect(() => {
    const categoryBasedOnCurrentPath = categories.filter(
      (category) => category.slug === path
    )[0];

    setCategory(categoryBasedOnCurrentPath);
  }, [path, categories]);

  const handleChangeTasksOrder = (result: DropResult) => {
    if (!result.destination) return;

    const currentListTasks = tasks.filter(
      (task) => task.category.slug === path
    );
  ...
  return (
    <MainStyles theme={lightTheme} checked={checked}>
      <div id="tasks-area-wrapper">
        ...
        <h1 className="category-indicator">
          {categories.filter((category) => category.slug === path)[0]?.title}
        </h1>

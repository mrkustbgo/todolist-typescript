...
const Category = ({ category }: CategoryComponentProps) => {
  ...
  const [slugBeingUpdated, setSlugBeingUpdated] = useState<string>("");
  ...
  const removeTasks = (slug: string) => {
    tasks.forEach((task) =>
      task.category.slug === slug ? removeTask(task.id) : ""
    );
  };
  ...
  const handleRemoveCategory = (slug: string, e: React.MouseEvent) => {
    e.stopPropagation();
  ...
  const taskAmount = tasks.filter(
    (task) => task.category.slug === category.slug
  ).length;
